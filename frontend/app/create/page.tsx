...
export default function CreateAgentPage() {
  ...
  useEffect(() => {
    ...
    if (savedAgentId && savedAgentId !== "momentum-base-1") {
      setAgentId(savedAgentId);
    } else if (!savedAgentId) {
      const rand = Math.floor(1000 + Math.random() * 9000);
      setAgentId(`momentum-${rand}`);
    }

    if (savedName && savedName !== "Momentum Bot") {
      setName(savedName);
    } else if (!savedName) {
      const rand = Math.floor(1000 + Math.random() * 9000);
      setName(`Momentum Trend Follower #${rand}`);
    }
  ...
  const activeTemplate =
    STRATEGY_TEMPLATES.find((t) => t.id === selectedTemplateId) || STRATEGY_TEMPLATES[0];
  ...
  function handleSelectTemplate(template: StrategyTemplate) {
    ...
    const isAutoId = !agentId || /^(momentum|mean-reversion|grid)-\d+$/.test(agentId);
    const rand = Math.floor(1000 + Math.random() * 9000);
    if (isAutoId) {
      const newId = `${template.id}-${rand}`;
      handleAgentIdChange(newId);
    }

    const isAutoName = !name || /^(Momentum Trend Follower|RSI Mean Reversion|Automated Grid Trading)( #\d+)?$/.test(name);
    if (isAutoName) {
      const newName = `${template.name} #${rand}`;
      handleNameChange(newName);
    }
  }

  function randomizeAgentId() {
    const rand = Math.floor(1000 + Math.random() * 9000);
    const prefix = activeTemplate.id;
    handleAgentIdChange(`${prefix}-${rand}`);
    const isAutoName = !name || /^(Momentum Trend Follower|RSI Mean Reversion|Automated Grid Trading)( #\d+)?$/.test(name);
    if (isAutoName) {
      handleNameChange(`${activeTemplate.name} #${rand}`);
    }
  }
    ...
    <Section className="pt-6 pb-20 sm:pt-8 relative z-10">
      ...
      {fromBacktest && backtestParams && (
        <div className="mb-8 rounded-xl border border-black/10 bg-neutral-200/60 p-1 dark:border-[#262626] dark:bg-[#141414]">
          <div className="flex items-start gap-4 rounded-lg bg-white px-5 py-4 dark:bg-[#0a0a0a]">
      ...
      <div className="mb-8 border-b border-border pb-6">
        ...
        <div className="rounded-xl border border-black/10 bg-neutral-200/60 p-1 dark:border-[#262626] dark:bg-[#141414]">
          <div className="flex flex-col gap-4 rounded-lg bg-white p-4 sm:p-5 dark:bg-[#0a0a0a]">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="grid w-full grid-cols-2 rounded-full border border-border bg-neutral-100 p-1 sm:w-auto dark:bg-[#141414]">
                <button
                  type="button"
      ...
      {((activeChain === "solana" && !publicKey) ||
        ...
        <div className="mb-8 rounded-xl border border-black/10 bg-neutral-200/60 p-1 dark:border-[#262626] dark:bg-[#141414]">
          <div className="flex flex-col gap-4 rounded-lg bg-white p-4 text-left sm:flex-row sm:items-center sm:justify-between dark:bg-[#0a0a0a]">
      ...
      <RevealSection>
        <div className="flex items-center justify-between mb-3">
          ...
          {(agentId || name || vaultPubkey) && (
            <button
              type="button"
              onClick={handleClearForm}
              title="Clear all fields"
              aria-label="Clear all fields"
              className="inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 font-mono text-[11px] text-foreground-faint hover:text-negative transition-colors cursor-pointer"
            >
        ...
        <div className="rounded-xl border border-black/10 bg-neutral-200/60 p-1 dark:border-[#262626] dark:bg-[#141414]">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5 rounded-lg bg-white p-6 dark:bg-[#0a0a0a]">
            <Field
              label="Agent ID"
              hint={`Unique per your wallet, immutable, up to ${MAX_AGENT_ID_LEN} bytes.`}
              value={agentId}
              onChange={handleAgentIdChange}
              placeholder={`e.g. ${activeTemplate.id}-1`}
              onRefresh={randomizeAgentId}
            />
            <Field
              label="Name"
              ...
              placeholder={`e.g. ${activeTemplate.name}`}
            />
            <Field
              label="Strategy URI"
              ...
              placeholder={activeTemplate.strategyUri}
            />
            {!isPaperMode && (
              <Field
                ...
                placeholder={activeChain === "solana"
                  ? (publicKey ? `${publicKey.toBase58().slice(0, 6)}...${publicKey.toBase58().slice(-4)} (Your connected wallet)` : "Connected wallet")
                  : (evmAddress ? `${evmAddress.slice(0, 6)}...${evmAddress.slice(-4)} (Your connected EVM wallet)` : "Connected EVM wallet")
                }
              />
            )}

            {errors.length > 0 && (
              <div className="rounded-lg border border-negative/30 bg-negative/10 p-4 text-negative">
            ...
            <AnimatePresence mode="wait" initial={false}>
              ...
              {state.status === "confirmed" && state.alreadyExisted && (
                <ResultTransition reduced={reduced} key="already-existed">
                  <div className="rounded-xl border border-black/10 bg-neutral-200/60 p-1 dark:border-[#262626] dark:bg-[#141414]">
                    <div className="flex flex-col items-center gap-3 rounded-lg bg-white py-6 text-center dark:bg-[#0a0a0a]">
              ...
              {state.status === "confirmed" && !state.alreadyExisted && (
                <ResultTransition reduced={reduced} key="registered">
                  <div className="rounded-xl border border-positive/30 bg-positive/10 p-1 dark:border-positive/30 dark:bg-positive/10">
                    <div className="flex flex-col items-center gap-3 rounded-lg bg-white py-6 text-center dark:bg-[#0a0a0a]">
    ...
    <div>
      ...
      <div className="relative mt-1.5">
        <input
          ...
          className={`w-full rounded-lg border border-border bg-neutral-50 px-3.5 py-2 font-mono text-xs text-foreground placeholder:text-foreground-faint transition-colors focus:border-accent focus:bg-white focus:outline-none dark:bg-black dark:focus:bg-[#0a0a0a] ${
            onRefresh && value ? "pr-14" : onRefresh || value ? "pr-8" : ""
          }`}
        />
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
          {value && (
            <button
              ...
              className="flex items-center justify-center h-5 w-5 rounded text-foreground-faint hover:text-foreground hover:bg-black/5 dark:hover:bg-white/10 transition-colors cursor-pointer"
            >
