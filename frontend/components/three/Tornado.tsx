...
// Tornado — Originkit
// Dropped in as supplied; the only local edit is hoisting the "use client"
// directive above the header comment, since Next requires it to be the first
...
function createVortex(
  ...
  function build() {
    ...
    const blendingMode = cfg.isLight ? THREE.NormalBlending : THREE.AdditiveBlending;
...
export default function Vortex(props: VortexProps) {
  ...
  React.useEffect(() => {
    ...
    // The canvas is created here rather than rendered by React, so every
    // mount gets a clean one. dispose() ends with forceContextLoss(), and a
    // canvas whose context has been released will never hand out another —
    // so a single JSX <canvas> reused across React's StrictMode
    // mount/unmount/mount is dead on the second pass, which Chrome paints as
    // a broken-image placeholder. Production never showed it because
    // StrictMode only double-invokes in development.
    ...
    try {
      ...
      // render). Fail to an empty panel rather than taking the page down.
